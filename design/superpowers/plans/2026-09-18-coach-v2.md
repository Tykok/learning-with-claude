...
- Modify: `uninstall.sh` (both by-name hook lists), `docs/install.html` (the "what gets installed" table), and whichever files state the hook count in prose
- Test: `test.sh`
...
**Shipping a ninth hook has inventory consequences, and this task owns all of them.** `test.sh:3146` derives the hook count from the filesystem and asserts the prose word matches everywhere it appears; `install.html`'s table must name every shipped hook file individually (that assertion exists because `coach-watch.sh` was once missing from it); and `uninstall.sh` removes hooks **by name** at lines ~130-140 and ~171, so a hook absent from those two lists survives an uninstall.

- [ ] Add `coach-armed-check.sh` to both by-name lists in `uninstall.sh`.
- [ ] Add a `hooks/coach-armed-check.sh` row to `docs/install.html`'s "what gets installed" table, matching the existing rows' markup.
- [ ] Run `bash test.sh 2>&1 | grep FAIL` and fix every hook-count word the suite names — the count goes from eight to nine, and the same number appears in more than one file.

Do not hand these to Task 7: they are consequences of creating a file, and the suite is the oracle for which files say a number.

- [ ] **Step 9: Commit**

```bash
git add plugins/learner/hooks/coach-armed-check.sh plugins/learner/hooks/coach-watch.sh plugins/learner/hooks/hooks.json plugins/learner/hooks/settings.snippet.json plugins/learner/hooks/learner-cleanup.sh plugins/learner/skills/status/SKILL.md uninstall.sh docs/install.html test.sh
...
- [ ] `grep -rn "coachCadence\|coachWorkMinutes\|coachWorkGrowthMinutes\|coachWorkMaxMinutes\|coachChallengeMinutes\|coachIdleCycles\|coachLines\|coachFiles\|coachEveryMinutes\|learner_coach_work_minutes\|coach-empty" plugins/ docs/ README.md learner.json.example` returns nothing outside `design/` — **with one intended exception**: `plugins/learner/skills/learner/SKILL.md` names the v1 keys in the sentence telling Claude to report them as obsolete and ignored (§1.5's migration rule). That list is the feature, not a leftover. Every other hit is stale.
