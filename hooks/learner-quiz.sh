...
2. .claude/learner-recap.md — TABLEAU DE BORD LISIBLE pour le dev (sections « À améliorer » par domaine : Code, Architecture, Tests, CI/Build, Données & DB, Intégrations ; « Acquis » ; « Historique des sessions » : tableau Date|Domaine|Style|Verdict|Note). Tu l'ÉCRIS/mets à jour seulement, tu ne le LIS JAMAIS pour choisir une question.
APRÈS la réponse de l'utilisateur, mets à jour les DEUX : dans learner-memory.md (ajout/retrait du point faible), et dans learner-recap.md (ajoute une ligne d'historique « | $TODAY | <domaine> | <code/archi/trou> | <✅ ok / ⚠️ à revoir / ⏭️ skip> | <note courte> | » + reflète le point en « À améliorer » ou « Acquis »). Garde tout concis."
